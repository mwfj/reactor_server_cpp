...
// contract of `KillOutstandingSnapshots` so future refactors that
// re-introduce a CASE-B per-dispatcher EnQueue marshal are caught.
//
// What we lock in:
//   * `kill_marshals_in_flight_` is RESERVED (never incremented today).
...
inline void TestKillMarshalReservedStaysZero() {
    std::cout << "\n[TEST] KillMarshal: kill_marshals_in_flight stays RESERVED"
              << std::endl;
    try {
        auto m = MakeManager();
        bool zero_before = m->kill_marshals_in_flight() == 0;

        // Register + kill — exercises the kill path that the future
        // marshal would bump if reintroduced.
        ...
        else if (!zero_after) err = "counter non-zero after kill (RESERVED contract violated)";
        else if (!finalized) err = "snapshot not finalized";
        TestFramework::RecordTest(
            "KillMarshal: kill_marshals_in_flight RESERVED stays zero",
