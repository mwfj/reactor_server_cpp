...
void TestStreamCloseSkipsNullOwnerCallback() {
    std::cout << "\n[TEST] Http2ConnectionHandler internal: stream close skips null owner callback..."
              << std::endl;
    try {
        int fds[2] = {-1, -1};
        if (::socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
        ...
        if (handler->pending_stream_close_cb_) {
            handler->pending_stream_close_cb_(nullptr, 1, 0);
        }

        if (close_cb_invoked) {
            pass = false;
            err += "original close callback invoked with null owner; ";
        }
        ...
        TestFramework::RecordTest(
            "Http2ConnectionHandler internal: stream close skips null owner callback",
            pass, err);
    } catch (const std::exception& e) {
        TestFramework::RecordTest(
            "Http2ConnectionHandler internal: stream close skips null owner callback",
...
void RunAllTests() {
    ...
    TestStreamCloseSkipsNullOwnerCallback();
}
