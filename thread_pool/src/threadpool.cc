...
inline void ThreadPool::SetThreadWorkerNum(int nums, bool set_by_init){
    thread_nums = nums;
    if(!set_by_init)
        logging::Get()->info("Set Max Worker Number to: {}", nums);
}
...
void ThreadPool::Init(){
    ...
    if(suggested_workers == 0) {
        logging::Get()->warn("Invalid core numbers, set to default number: {}", DEFAULT_THREAD_NUMS);
...
void ThreadPool::Start(){
    ...
    logging::Get()->debug("ThreadPool Start");
}
...
void ThreadPool::Stop(){
    ...
    logging::Get()->debug("ThreadPool Stopped");
}

void ThreadPool::Run() {
    logging::Get()->debug("ThreadPool worker running");
    ...
    logging::Get()->debug("ThreadPool worker exiting");
}
