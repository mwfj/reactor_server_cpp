...
- **H2 send-stall is a timeout** — if the request body is large enough to span multiple DATA frames, the per-stream send-stall budget refreshes on each DATA flush (mirroring H1's `SetWriteProgressCb` behavior). A truly stalled upload — peer's flow-control window drained and not refreshing for `response_timeout_ms` (or `30s` if disabled) — surfaces as `RESULT_RESPONSE_TIMEOUT` (504), not `RESULT_UPSTREAM_DISCONNECT` (502). This matches H1's send-stall semantics and routes through the retryable-timeout path so `retry_on_timeout` policies apply.
